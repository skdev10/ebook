...
namespace EBookDashboard.Controllers
{
    public class PlansController : Controller
    {
        ...
        public PlansController(IPlansService planService, IHttpClientFactory httpClientFactory, ApplicationDbContext context, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> BuyPlan(int planId)
        {
            ...
            var paymentApiBase = _configuration["PaymentApi:BaseUrl"]?.Trim().TrimEnd('/');
            if (string.IsNullOrWhiteSpace(paymentApiBase))
                return BadRequest("Payment API is not configured. Set PaymentApi:BaseUrl (or environment variable PaymentApi__BaseUrl).");

            var client = _httpClientFactory.CreateClient();
            client.BaseAddress = new Uri(paymentApiBase + "/");
